...
function secret() {
  const s = process.env.AUTH_SECRET || process.env.CAPTION_AUTH_SECRET;
  if (!s) {
    throw new Error('SECURITY ERROR: AUTH_SECRET or CAPTION_AUTH_SECRET environment variable is required. Authentication will not function without a secure secret.');
  }
  return s;
}
